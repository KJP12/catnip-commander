...
public class HelpCommand extends AbstractCommand implements IViewable {
    ...
    public void view(Message msg) {
        ...
        channel.sendMessage("Generating Help...").handleAsync((m, t) -> {
            ...
            if(catSys.SYSTEM_CATEGORY.checkPermission(msg, this, false)) allowed.add(catSys.SYSTEM_CATEGORY); else return m.edit("Command system is locked down. How did you execute this?");
            var catMap = new TreeMap<String, String>(String.CASE_INSENSITIVE_ORDER);

            for(var cat:catList) try {
            ...
            for(var cat:allowed){
                ...
                cmd: for(var cmd: cmds){
                    if(cmd.isHidden() || !allowed.containsAll(cmd.getCategoryList())) continue;

                    try{
                        if(cmd.checkRuntimePermission(msg, false)) sortedCmds.add((noError && cmd.checkRuntimePermission(m, false)) ? cmd.getFirstAliases() : cmd.getFirstAliases() + "*");
                ...
                if(!sortedCmds.isEmpty()){
                    if(sbCmd == null) sbCmd = new StringBuilder(sortedCmds.size() * 18); else sbCmd.setLength(0);
                    if(sbCat == null) {
            ...
            if(isEmbed) {
                ...
                sb.append('\n');
                for(var ess: catMap.entrySet()) sb.append(ess.getKey()).append('\n').append(ess.getValue()).append("\n\n");
    ...
    public void run(Message msg, String args) throws Throwable {
        ...
        if(cmd != null) chan.sendMessage(cmd.toMessage(msg)); else chan.sendMessage("Command wasn't found.");
    }
